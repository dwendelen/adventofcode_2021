...
(defn process [lines emp]
  (let [ n (count lines)
         ...
         new-empty (get table (toIdx (repeat 9 emp)))
       ]
    [new-line new-empty]
  )
)
...
; not 19578 is too high
(let [ [p1 e1] (reduce (fn [[p e] _] (process p e)) [initial-image 0] (range 50))
       m1 (map #(reduce + %) p1)
       r (reduce + m1)
     ]
  (println (count p1) (count (first p1)))
  (println r)
)
