...
(defn increaseLine [i line]
  (map #(inc (mod (+ i (dec %)) 9))  line)
)

(defn makeLineWide [line i]
  (vec (mapcat #(increaseLine (+ i %) line) (range 5)))
)

(defn makeGridWide [grid i]
  (map #(makeLineWide % i) grid)
)

(def bigGrid
  (vec (mapcat #(makeGridWide grid %) (range 5)))
)
...
(def res
  (let [ ny (count bigGrid)
         nx (count (first bigGrid))
         ...
         result (dijkstra initialTodo [(dec ny) (dec nx)] bigGrid)
       ]
    result
  )
)
